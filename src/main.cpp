...
void open_plugin_gui(const std::string& plugin_path) {
  ...
  if (!window_result) {
    log.error("failed to create editor window", redlog::field("error", window_result.error()));
    return;
  }
...
void process_audio_file(const std::string& input_file,
  ...
  log.inf("processing audio file", 
          redlog::field("input", input_file),
  ...
  log.inf("plugin loaded for audio processing", redlog::field("name", plugin.name()));
  log.warn("audio file processing not yet implemented - placeholder functionality");
  log.inf("would process:", 
          redlog::field("input_file", input_file),
          redlog::field("output_file", output_file),
          redlog::field("plugin_name", plugin.name()));
}

void cmd_inspect(args::Subparser& parser) {
  args::Positional<std::string> plugin_path(parser, "plugin_path", "path to vst3 plugin to inspect");
  parser.Parse();
...
void cmd_gui(args::Subparser& parser) {
  args::Positional<std::string> plugin_path(parser, "plugin_path", "path to vst3 plugin to open in gui");
  parser.Parse();
...
void cmd_process(args::Subparser& parser) {
  args::ValueFlag<std::string> input_file(parser, "input", "input audio file", {'i', "input"});
  args::ValueFlag<std::string> output_file(parser, "output", "output audio file", {'o', "output"});
  args::Positional<std::string> plugin_path(parser, "plugin_path", "path to vst3 plugin to use for processing");
  parser.Parse();
  
  if (!plugin_path || !input_file || !output_file) {
    log_main.error("plugin path, input file, and output file required for process command");
    std::cerr << parser;
    return;
  }
  
  process_audio_file(args::get(input_file), args::get(output_file), args::get(plugin_path));
}

void cmd_scan(args::Subparser& parser) {
  args::ValueFlagList<std::string> search_paths(parser, "paths", "additional search paths", {'p', "path"});
  args::Flag detailed(parser, "detailed", "show detailed plugin information", {'d', "detailed"});
  parser.Parse();
...
int main(int argc, char* argv[]) {
  ...
  args::CounterFlag verbosity(arguments, "verbosity", "verbosity level", {'v'});

  args::ArgumentParser parser("vstshill - cross-platform vst3 host", "analyze, host, and process vst3 plugins");
  ...
  args::Command inspect_cmd(commands, "inspect", "inspect and analyze a vst3 plugin", &cmd_inspect);
  args::Command gui_cmd(commands, "gui", "open plugin editor gui window", &cmd_gui);
  args::Command process_cmd(commands, "process", "process audio files through plugin", &cmd_process);
  args::Command scan_cmd(commands, "scan", "scan for vst3 plugins in standard directories", &cmd_scan);
  ...
  try {
    ...
    log_main.inf("vstshill: cross-platform vst3 host");
    log_main.inf("usage examples:");
    log_main.inf("  vstshill inspect <plugin_path>     # analyze plugin");
    log_main.inf("  vstshill gui <plugin_path>         # open editor gui");
    log_main.inf("  vstshill process -i in.wav -o out.wav <plugin_path>  # process audio");
    log_main.inf("  vstshill scan                      # find all plugins");
    
    log_main.inf("vst3 search directories:");
    for (const auto& path : vstk::util::get_vst3_search_paths()) {
      if (!path.empty()) {
        log_main.inf("  " + path);
      }
    }
  ...
  int verb_level = args::get(verbosity);
  switch (verb_level) {
  case 0:
    redlog::set_level(redlog::level::info);
    break;
  ...
  log_main.trc("verbosity level set",
               ...
               redlog::field("flag_count", verb_level));
